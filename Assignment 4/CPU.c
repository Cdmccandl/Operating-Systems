...
//Collaborated with Dominick Nola and John Shapiro when stuck.
/*
...
struct PCB {
    ...
    int ended;          // the time this process ended
};
...
void send_signals(int signal, int pid, int interval, int number) {
    ...
    WRITESTRING("<<< send_signals exiting\n");
}
...
void scheduler (int signum) {
    ...
    while(found_new == 0 && proc_num < numOfProcesses) {


        if(processes[proc_num].state == NEW) {
    ...
    if(found_new){
        running = &processes[proc_num];
        running->state = RUNNING;
        assert((processes[proc_num].started = time(NULL)) > 0);
        ...
        if(running->pid < 0){
            ...
            exit(errno);
                ...
                execl(running->name, "process name", NULL);
                perror("execl error");
                exit(errno);
        } else{
        }
        if (running->pid != processes[proc_num].pid){
            processes[proc_num].switches++;
        }
        running = &processes[proc_num];
    }
    else {
        if(round_robin_process == numOfProcesses){
            round_robin_process = 0;
        }
        if(processes[round_robin_process].state == READY)
        {
            if (running->pid != processes[round_robin_process].pid)
            {
                running->switches++;
            }
            running = &processes[round_robin_process];
...
void process_done (int signum) {
    ...
    if (running->pid != idle.pid){
        ...
        WRITESTRING("Final IDLE until execution time complete!!\n")
    }

    WRITESTRING("Exiting process_done...\n");
